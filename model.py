...
#         rfeat = self.rmodule(rels)
#         logit = (self.lin(pair) * rfeat).sum(-1)
#         # logit = (self.lin(pair) * rfeat)
#         #logit = self.cls(logit)
#         # print(logit.shape)
#         return logit
...
#
#         pair = torch.cat([h_final, t_final], dim=-1)
#         pair = torch.cat([pair, pair_conv], dim=-1)
#         #print(pair.shape)
#
#         rfeat = self.rmodule(rels)
#         logit = (self.lin(pair) * rfeat).sum(-1)
...
#
#
# #cnn and graph trilinear attention
# class SA_DDI(torch.nn.Module):
#     def __init__(self, in_dim, edge_in_dim, hidden_dim=160, n_iter=10):
#         super(SA_DDI, self).__init__()
#
#         self.drug_encoder = DrugEncoder(in_dim, edge_in_dim, hidden_dim, n_iter=n_iter)
#         self.h_gpool = GlobalAttentionPool(hidden_dim)
...
#         self.fc1 = nn.Linear(320, 128)
#         #cnn
#         self.CQA_attention = CQAttention(160)
#         self.cls = nn.Linear(hidden_dim,2)
#
#     def forward(self, triples,head_embedding,tail_embedding):
#         h_data, t_data, rels = triples
...
#         headsConv = headsConv.unsqueeze(0)
#
#         t_final = t_final.unsqueeze(0)
#         tailsConv = tailsConv.unsqueeze(0)
#
#         pair_head = self.CQA_attention(h_final,headsConv).squeeze(0)
#         pair_tail = self.CQA_attention(t_final,tailsConv).squeeze(0)
#         # print(pair_head.shape)
#         # print(pair_tail.shape)
#
#
#
#
...
#         # pair = torch.cat([pair, pair_conv], dim=-1)
#         # print(pair.shape)
#         pair = torch.cat([pair_head, pair_tail], dim=-1)
#         rfeat = self.rmodule(rels)
#         #print(pair.shape)
#         logit = (self.lin(pair) * rfeat).sum(-1)
#         return logit
...
#
#
#         # print("pair_conv",pair_conv.shape)
#
#         # sag_heads = self.sag_encode(h_data)
#         # sag_tails = self.sag_encode(t_data)
#         # pair_sag = torch.cat([sag_heads, sag_tails], dim=1)
#
#         # print("sag_heads",sag_heads.shape)
#         # print("sag_tails",sag_tails.shape)
#         # print("pair_sag",pair_sag.shape)
#
#
#
...
class SA_DDI_kge(torch.nn.Module):
    def __init__(self, in_dim, edge_in_dim, hidden_dim=64, n_iter=10):
        super(SA_DDI_kge, self).__init__()
